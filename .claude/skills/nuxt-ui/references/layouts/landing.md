...
- Forgetting the `#body` slot on `UHeader` — this is the mobile menu content.
  Without it, mobile users have no navigation.
- Using `variant="solid"` for both header and hero buttons — the header button
  should be lower weight than the hero CTA.
...
- `UFooterColumns` — multi-column footer with link groups (used inside
  `UFooter`).
