...
Nuxt UI automatically registers `@nuxt/icon`, `@nuxt/fonts`, and
`@nuxtjs/color-mode`. Do **not** add them to your `modules` array. Configure
them via root-level keys in `nuxt.config.ts`:
...
When using `@nuxt/content`, it **must** come after `@nuxt/ui` in the `modules`
array — otherwise prose components won't be available:
...
Use `mapContentNavigation` to transform content navigation for components like
`UBreadcrumb`:
...
Recommended `.vscode/settings.json` for Tailwind IntelliSense autocomplete with
Nuxt UI:
...
Nuxt UI registers `@nuxt/icon` automatically. Format: `i-{collection}-{name}`.
Prefer `lucide` collection.
...
Components like `Modal`, `Select`, `Accordion`, etc. use default icons from
`appConfig.ui.icons`. Override them globally:
...
Components supporting nested arrays: `UDropdownMenu`, `UContextMenu`,
`UCommandPalette`, `UNavigationMenu`.
...
Programmatic modals, slideovers, drawers — no template `v-model` needed. See
[overlays recipe](../recipes/overlays.md) for full patterns.
...
Nuxt UI supports 50+ locales. Set the locale on `UApp` — all components inherit
it.
...
`extendLocale` is auto-imported. Override specific messages or the `code`
(affects date/time formatting in Calendar, InputDate, InputTime):
...
Each locale has a `dir` property (`'ltr'` or `'rtl'`). `UApp` uses it to set
directionality on all components. Use `useHead` to propagate `lang` and `dir` to
the `<html>` element.

## Color mode

Nuxt UI registers `@nuxtjs/color-mode` automatically. Built-in components for
switching:
...
For custom color mode UI, use `useColorMode` with `ClientOnly` to avoid
hydration mismatch:
...
- Dashboard sidebar hides on mobile, shows a slideover/drawer via
  `UDashboardSidebar` `mode` prop
- `UHeader` body slot is the mobile menu content (shown when hamburger is
  tapped)
- Most components handle responsiveness automatically — avoid manual breakpoint
  classes unless needed
